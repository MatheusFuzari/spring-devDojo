...
class UserServiceTest {
    ...
    void findById_ThrowsResponseStatusException_WhenIdIsNotFound() {
        ...
        Assertions.assertThatException().isThrownBy(() -> service.findByIdOrThrowNotFound(99L)).isInstanceOf(ResponseStatusException.class);
    }
    ...
    void save_CreatesUser_WhenSuccessful() {
        ...
        BDDMockito.when(service.save(userToSave)).thenReturn(userUtils.newUserSaved());
        ...
        Assertions.assertThat(userCreated).hasNoNullFieldsOrProperties().isEqualTo(userUtils.newUserSaved());
    }
    ...
    void save_ThrowsResponseStatusException_WhenEmailAlreadyExists() {
        ...
        var userToSave = new User().withId(4L).withEmail("isagi.yoichi@fromblue.com").withFirstName("Isagi").withLastName("Yoichi");

        BDDMockito.when(repository.findByEmail(emailAlreadyExistent.getEmail())).thenReturn(Optional.of(emailAlreadyExistent));

        Assertions.assertThatException().isThrownBy(() -> service.save(userToSave)).isInstanceOf(ResponseStatusException.class);
    }
    ...
    void update_UpdateUser_WhenIdExists() {
        var userToUpdate = userList.getFirst().withEmail("isagi.yoichi@fromblue.com");
